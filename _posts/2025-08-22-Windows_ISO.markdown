...
追記：この方法では、インストールできなくなった（と考えるのが妥当）。OSをインストールするとブートローダーが筐体ディスクにインストールされない。ブートメディアなしでは、起動しなくなる。ブートローダーを手動でインストールすれば、問題なくなる。面倒な場合は、メディアを作るときに、Linux(woeusb)を使えば解決する。
