...
***SHELL-ish（シェリッシュ） Excel VBA***は、造語である。SHELL-ishであることから、シェルのようにテキストを扱う。インプットをテキストとして、アウトプットをテキストファイルとする。

シェルのパイプで繋いだ標準出力をリダイレクトでファイル出力するイメージを、Excel VBAで行う。
...
- 文字列の合致比較・条件分岐は、`If str Like "*World" Then`が役立つ。
